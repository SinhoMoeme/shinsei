...
pystack_t* shinsei_ex_pystack_t_con(){
	register pystack_t *const res=(pystack_t *const)malloc(sizeof(pystack_t));
	...
	do{
		...
		if(__builtin_expect(res->data!=nullptr,1)) break;
		cap>>=1;
	}
	while(cap);
	if(__builtin_expect(res->data==nullptr,0)){
		free(res);
		return nullptr;
	}
	res->size=0;
...
pystack_t* shinsei_ex_pystack_t_conAssign(PyObject **const restrict data,const size_t size,const size_t cap){
	register pystack_t*const res=(pystack_t*const)malloc(sizeof(pystack_t));
	if(__builtin_expect(res==nullptr,0)) return nullptr;
	res->data=data;
	res->size=size;
...
bool shinsei_ex_pystack_t_shrink(pystack_t*const restrict this,const size_t less_cap){
	...
	register PyObject **const ptr=(PyObject**const)realloc(this->data,new_size*sizeof(PyObject*));
	if(__builtin_expect(ptr==nullptr,0)) return false;
	this->cap-=less_cap;
	this->data=ptr;
	return true;
