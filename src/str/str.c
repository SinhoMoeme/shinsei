...
bool shinsei_isBDigitU8S(const char8_t**restrict str_ptr,const size_t len){
...
bool shinsei_isBDigitU16S(const char16_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
...
bool shinsei_isODigitU8S(const char8_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf8CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isODigitU16S(const char16_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf16CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isDigitU8S(const char8_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf8CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isDigitU16S(const char16_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf16CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isXDigitU8S(const char8_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf8CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isXDigitU16S(const char16_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf16CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isUpperU8S(const char8_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf8CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isUpperU16S(const char16_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf16CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isLowerU8S(const char8_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf8CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isLowerU16S(const char16_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf16CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isAlphaU8S(const char8_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf8CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isAlphaU16S(const char16_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf16CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isAlnumU8S(const char8_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf8CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
bool shinsei_isAlnumU16S(const char16_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf16CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
	*str_ptr+=size;
...
char8_t shinsei_toUpperU8S(const char8_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf8CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
...
char16_t shinsei_toUpperU16S(const char16_t**restrict str_ptr,const size_t len){
	if(__builtin_expect(!len,0)) return false;
	register const int size=shinsei_utf16CharSize(**str_ptr);
	if(__builtin_expect(len<size,0)) return false;
...
int shinsei_utf32CharToUtf8Char(char8_t*restrict des,const char32_t src){
	if(src<=0x7F){
		des[0]=(char)src;
		return 1;
	}
...
int shinsei_utf16CharToUtf8Char(char8_t*restrict des,const char16_t*restrict src,const size_t src_len){
...
int shinsei_utf8CharToUtf32Char(char32_t*restrict des,const char8_t* src,const size_t src_len){
	if(__builtin_expect(!src_len,0)) return 0;
	if(src[0]<0x80){
		*des=(char32_t)src[0];
		return 1;
...
int shinsei_utf8CharToUtf16Char(char16_t*restrict des,const char8_t* src,const size_t src_len){
	char32_t buf;
...
size_t shinsei_utf16ToUtf8(char8_t *restrict des,const size_t des_len,const char16_t *restrict src,const size_t src_len){
	size_t si=0;
	size_t di=0;
	while(si<src_len){
		char32_t cp;
		register int r=shinsei_utf16CharToUtf32Char(&cp,src+si,src_len-si);
		if(__builtin_expect(!r,0)) return 0;
		si+=r;
		char8_t tmp[4];
		int w=shinsei_utf32CharToUtf8Char(tmp,cp);
		if(di+w>des_len) return di+w;
		for(int i=0;i<w;i++) des[di++]=tmp[i];
	}
	if(di<des_len) des[di]=0;
	return di;
}
char8_t* shinsei_utf16ToUtf8B(const char16_t *src,const size_t src_len){
	size_t cap=src_len*4+1;
	char8_t* buf=(char8_t*)malloc(cap);
	if(!buf) return nullptr;
	size_t len=shinsei_utf16ToUtf8(buf,cap,src,src_len);
	if(!len){
		free(buf);
		return nullptr;
	}
