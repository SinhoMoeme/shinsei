...
static uint_fast32_t _sleep_ms_fls=FLS_OUT_OF_INDEXES;
static INIT_ONCE _sleep_ms_init_once=INIT_ONCE_STATIC_INIT;
static int __stdcall sleepMsInitOnce(INIT_ONCE* init_once,void* para,void** content){
	(void)init_once;
	(void)para;
	(void)content;
	_sleep_ms_fls=FlsAlloc(flsCloseHandle);
	if(__builtin_expect(_sleep_ms_fls==FLS_OUT_OF_INDEXES,0)) return 0;
	return 1;
}
bool shinsei_sleepMs(unsigned long long ms){
	...
	void* timer=(HANDLE)FlsGetValue(_sleep_ms_fls);
    ...
    if(__builtin_expect(SetWaitableTimer(timer,&due,0,nullptr,nullptr,0)==0,0)) return false;
